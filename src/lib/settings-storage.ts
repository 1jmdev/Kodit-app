...
export function loadStoredSettings(): StoredSettings {
    ...
    const normalizedSelectedModelId = parsed.data.selectedModelId.includes(":")
        ...
        : getModelKey(
              parsed.data.modelProfiles.find(
                  (model) => model.id === parsed.data.selectedModelId,
              ) ?? parsed.data.modelProfiles[0],
          );

    if (
        !parsed.data.modelProfiles.some(
            (model) => getModelKey(model) === normalizedSelectedModelId,
        )
    ) {
        return {
            ...parsed.data,
            selectedModelId: getModelKey(parsed.data.modelProfiles[0]),
        };
    }
