...
export function SettingsPage() {
    ...
    function fuzzyScore(needle: string, haystack: string): number {
        ...
        for (let i = 0; i < needle.length; i += 1) {
            ...
            if (
                ...
                foundIndex === previousMatchIndex + 1
            ) {
                score += 2;
            }
            if (
    ...
    async function handleRefreshModels(providerId: string) {
        ...
        if (!validation.success) {
            setProviderErrorById((prev) => ({
                ...
                [providerId]:
                    validation.message ||
                    getProviderPreset(providerId).apiKeyHint,
            }));
            return;
        }
    ...
    function handleAddModelProfile(model: ModelConfig) {
        if (
            state.settings.modelProfiles.some(
            ...
            <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 pb-16">
                <div className="space-y-2">
                    ...
                    <p className="text-sm text-muted-foreground">
                        Configure provider presets, model profiles, and your
                        workspace behavior.
                    </p>
                </div>
                ...
                <Card className="overflow-visible">
                    <CardHeader>
                        ...
                        <CardDescription>
                            Customize how window controls (minimize, maximize,
                            close) appear in the title bar.
                        </CardDescription>
                    </CardHeader>
                        ...
                        <Card key={preset.id}>
                            <CardHeader>
                                ...
                                <CardDescription>
                                    ...
                                    <code>
                                        ~/.config/com.1jmdev.kodit/auth.json
                                    </code>
                                    .
                                </CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                <div className="space-y-2">
                                    <Label htmlFor={`${preset.id}-api-key`}>
                                        API key
                                    </Label>
                                    <div className="flex flex-col gap-2 sm:flex-row">
                                        ...
                                        <Button
                                            ...
                                            onClick={() =>
                                                void handleSaveApiKey(preset.id)
                                            }
                                            disabled={
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            The model selector now shows only profiles defined
                            here.
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <div className="max-h-[15.5rem] overflow-y-auto rounded-md border border-border/60 bg-muted/20 p-2">
                                {modelSearch.trim().length === 0 && (
                                    <p className="px-2 py-1 text-xs text-muted-foreground">
                                        Enter a search query and add models as
                                        profiles.
                                    </p>
                                )}
