...
export function SidebarNav({ onNewThread, onPrimaryMouseDown, onKeyboardClick }: SidebarNavProps) {
  return (
    <div className="flex flex-col gap-0.5 px-2 py-1">
      {navItems.map((item) => {
        ...
        return (
          <button
            key={item.id}
            onMouseDown={action ? (event) => onPrimaryMouseDown(event, action) : undefined}
            onClick={action ? (event) => onKeyboardClick(event, action) : undefined}
            className={cn(
