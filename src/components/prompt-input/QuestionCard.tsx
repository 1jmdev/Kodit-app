    ...
    <div className="rounded-xl border border-border/50 bg-card/50 backdrop-blur-sm overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-200">
        ...
        {question.options.map((opt) => (
          <button
            ...
            className={cn(
              ...
              selected.has(opt.label) ? "bg-accent text-foreground" : "hover:bg-accent/50 text-foreground/80",
            )}
          >
            <div className="flex items-center gap-2">
              {isMultiple && (
                <div className={cn("size-4 rounded border flex items-center justify-center shrink-0", selected.has(opt.label) ? "bg-foreground border-foreground" : "border-border")}>
      ...
      {isMultiple && selected.size > 0 && (
        <div className="px-4 pb-3">
          <Button size="sm" onClick={() => onAnswer(Array.from(selected))} className="w-full">
