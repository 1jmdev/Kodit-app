...
import { ChevronDown, ChevronRight, Circle, Folder, MessageSquare } from "lucide-react";
...
export function ProjectThreadsSection({
  ...
  const threadsByProject = projects.map((project) => ({
    project,
    threads: threads.filter((thread) => thread.projectId === project.id).sort((a, b) => b.updatedAt - a.updatedAt),
  }));
          ...
          return (
            <div key={project.id} className="mx-0.5">
              <div className="flex w-full items-center rounded-md text-left text-muted-foreground hover:bg-accent/30 hover:text-foreground transition-colors">
                <button
                  onClick={() => setExpandedProjects((prev) => ({ ...prev, [project.id]: !expanded }))}
                  className="px-2 py-1.5 shrink-0"
                  aria-label={expanded ? `Collapse ${project.name}` : `Expand ${project.name}`}
                >
                  {expanded ? <ChevronDown className="size-3 text-muted-foreground/60 shrink-0" /> : <ChevronRight className="size-3 text-muted-foreground/60 shrink-0" />}
                </button>
                <button
                  onMouseDown={(event) => onPrimaryMouseDown(event, () => {
                    onSelectProject(project.id);
                    navigate("/");
                  })}
                  onClick={(event) => onKeyboardClick(event, () => {
                    onSelectProject(project.id);
                    navigate("/");
                  })}
              ...
              {expanded && (
                <div className="pb-1 pl-6">
                  {shownThreads.map((thread) => {
                    const isActive = thread.id === activeThreadId && location.pathname.includes("/chat/");
                    return (
                      <button
                        key={thread.id}
                        onMouseDown={(event) => onPrimaryMouseDown(event, () => onSelectThread(thread.id))}
                        onClick={(event) => onKeyboardClick(event, () => onSelectThread(thread.id))}
                        className={cn(
                      ...
                      >
                        {isActive ? <Circle className="size-2 mt-1.5 shrink-0 fill-sky-500 text-sky-500" /> : <MessageSquare className="size-3.5 mt-0.5 shrink-0 opacity-45" />}
                        <div className="flex-1 min-w-0">
                          <span className="truncate text-[13px] font-medium leading-tight">{thread.title}</span>
                          <div className="mt-0.5 text-[11px] opacity-50">{formatTimeAgo(thread.updatedAt)}</div>
                        </div>
                      </button>
                    );
                  })}
                  ...
                  {threads.length > visible && (
                    <button
                      onClick={() => setVisibleThreadCount((prev) => ({ ...prev, [project.id]: (prev[project.id] ?? 4) + 10 }))}
                  ...
                  {visible > 4 && (
                    <button
                      onClick={() => setVisibleThreadCount((prev) => ({ ...prev, [project.id]: Math.max(4, (prev[project.id] ?? 4) - 10) }))}
                      className="mt-1 rounded-md px-2 py-1 text-xs text-muted-foreground hover:bg-accent/30 hover:text-foreground"
                    >
                      Show less
                    </button>
