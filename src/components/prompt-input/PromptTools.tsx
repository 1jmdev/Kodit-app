...
function IconTool({ label, icon: Icon }: { label: string; icon: ComponentType<{ className?: string }> }) {
  return (
    <Tooltip>
      <TooltipTrigger render={<Button variant="ghost" size="icon-xs" className="text-muted-foreground/70 hover:text-foreground" />}>
...
export function PromptTools({ selectedModel, modelProfiles, dispatch }: PromptToolsProps) {
  return (
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" side="top" sideOffset={8} className="max-h-72 w-80 overflow-y-auto">
          {modelProfiles.map((model) => (
            <DropdownMenuItem key={`${model.providerId}:${model.id}`} onClick={() => dispatch({ type: "SET_MODEL", model })}>
