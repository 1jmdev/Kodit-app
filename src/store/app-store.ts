                              ...
                              messages: t.messages.map((m) =>
                                  m.id === action.messageId
                                      ? {
                                            ...
                                            reasoning:
                                                action.reasoning ?? m.reasoning,
                                            isStreaming:
                                                action.isStreaming ??
                                                m.isStreaming,
                                            toolCalls:
                                                action.toolCalls ?? m.toolCalls,
                                            todos: action.todos ?? m.todos,
                                            questions:
                                                action.questions ?? m.questions,
                                        }
                                      : m,
                              ),
                          }
        ...
        case "SET_MODEL_PROFILES": {
            const dedupedProfiles = action.profiles.filter(
                (model, index, models) =>
                    models.findIndex(
                        (candidate) =>
                            getModelKey(candidate) === getModelKey(model),
                    ) === index,
            );
            const nextSelectedModelId =
                action.selectedModelId ?? state.settings.selectedModelId;
            const selectedModel =
                dedupedProfiles.find(
                    (model) => getModelKey(model) === nextSelectedModelId,
                ) ??
                dedupedProfiles[0] ??
                state.selectedModel;
