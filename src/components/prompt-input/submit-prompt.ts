...
export async function submitPrompt({
  ...
  const userMessage = await addMessage({ threadId, role: "user", content: userInput, mode: "build" });
  ...
  if (!keyValidation.success) {
    ...
    dispatch({ type: "REPLACE_MESSAGE", threadId, messageId: assistantMessageId, nextMessage: savedAgentMessage });
    return;
  }
  ...
  try {
    ...
    const streamResult = await streamProviderText({
      ...
      onChunk: (chunk) => dispatch({ type: "UPDATE_MESSAGE", threadId, messageId: assistantMessageId, content: chunk, isStreaming: true }),
      onReasoning: (reasoning) => dispatch({ type: "UPDATE_MESSAGE", threadId, messageId: assistantMessageId, reasoning, isStreaming: true }),
      onTodos: (todos) => dispatch({ type: "SET_THREAD_TODOS", threadId, todos }),
      onToolCalls: (toolCalls) => {
        dispatch({ type: "UPDATE_MESSAGE", threadId, messageId: assistantMessageId, toolCalls, isStreaming: true });
        const nextCompleted = toolCalls.filter((toolCall) => toolCall.status === "completed" && toolCall.name.startsWith("Edited ")).length;
    ...
    dispatch({ type: "SET_THREAD_TODOS", threadId, todos: streamResult.todos });
    dispatch({ type: "REPLACE_MESSAGE", threadId, messageId: assistantMessageId, nextMessage: savedAgentMessage });
