...
export function createEditTool(workspacePath: string) {
  return tool({
    ...
    inputSchema: z.object({
      path: z.string().min(1).describe("The relative path to the file to edit"),
      oldString: z.string().describe("The exact string to find in the file"),
      newString: z.string().describe("The string to replace oldString with (must be different from oldString)"),
      replaceAll: z
        .boolean()
        .optional()
        .default(false)
        .describe("Replace all occurrences of oldString in the file (default false)"),
    }),
    execute: async ({ path, oldString, newString, replaceAll }) => {
      if (oldString === newString) {
        return { ok: false, error: "oldString and newString are identical — no change needed." };
      }
